...
var default_alipay_config = {
  ...
  ,alipay_wap_trade_create_direct_notify_url: ''
  ,alipay_wap_trade_create_direct_return_url: ''
  ,alipay_wap_auth_authAndExecute_notify_url: ''
  ,alipay_wap_auth_authAndExecute_return_url: ''
...
Alipay.prototype.route = function(app){
  ...
  app.post(this.alipay_wap_trade_create_direct_notify_url, function(req, res){self.alipay_wap_trade_create_direct_notify(req, res)});
...
call_back_url
 */
Alipay.prototype.alipay_wap_trade_create_direct = function(data, res) {
  var alipaySubmit = new AlipaySubmit(this.alipay_config);
  var direct_trade_create_req = {};
  direct_trade_create_req.direct_trade_create_req = data;
  var parameter = {
    ...
    sec_id: 'MD5',
    req_data: json2xml(direct_trade_create_req)
  }

  var html_text = alipaySubmit.buildRequestForm(parameter,"get", "确认");
  var url = 'https://wappaygw.alipay.com/service/rest.htm?format=' +
    'xml' +
    '&partner=' + this.alipay_config.partner +
    '&req_data=' + encodeURIComponent(json2xml(direct_trade_create_req)) +
    '&req_id=1282889689836' +
    '&sec_id=MD5' +
    '&service=alipay.wap.trade.create.direct' +
    '&v=2.0' +
  ...
  request(url, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      var reg = /<\/?request_token>/;
//      console.log('body:\n',decodeURIComponent(body));
//      console.log('token: ',decodeURIComponent(body).split(reg)[1]);
      var token = decodeURIComponent(body).split(reg)[1];
      var auth_and_execute_req = {};
      auth_and_execute_req.auth_and_execute_req = {
        request_token: token
      }
      var param = {
        ...
        format: 'xml',
        v: '2.0',
        partner: '2088411764311875',
        sec_id:'MD5',
        req_data: json2xml(auth_and_execute_req)
      }
      var html_t = alipaySubmit.buildRequestForm(param, "get", "确认");
      res.send(html_t.form);
//      res.send(decodeURIComponent(body));
    }
  })
}
...
Alipay.prototype.alipay_wap_trade_create_direct_notify = function(req, res){
  console.log('alipay notify entered');
  var self = this;
  var _POST = req.body;
  var alipayNotify = new AlipayNotify(this.alipay_config);
  alipayNotify.verifyNotify(_POST, function(veryfy_result){
    if (verify_result) {
      console.log(_POST);
      var notify_data = parser.toJson(_POST.notify_url) || parser.toJson(JSON.parse(decodeURIComponent(_POST)).notify_url);
      if (notify_data.trade_status == 'TRADE_FINISHED' || notify_data.trade_status == 'TRADE_SUCCESS') {
        console.log('verified and trade success or finished');
        request.put('http://service.fankahui.com:2403/orders', {body: {
          id: notify_data.out_trade_no,
          payment: {
            type: 'alipay',
            status: 'paid'
          }
