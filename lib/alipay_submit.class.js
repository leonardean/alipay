...
function AlipaySubmit(alipay_config){
    ...
    this.alipay_gateway_wap = 'http://wappaygw.alipay.com/service/rest.htm?';
...
AlipaySubmit.prototype.buildRequestForm = function (para_temp, method, button_name) {
    ...
    var sHtml = {};
    sHtml.form = "<form id='alipaysubmit' name='alipaysubmit' action='"
            + this.alipay_gateway_wap
            +  "_input_charset="
    ...
    for(var key in para){
        var val = para[key];
      if (key != 'sign_type')
        sHtml.form += "<input type='hidden' name='" + key + "' value='" + val + "'/>";
      if (key == 'sign')
        sHtml.sign = val;
    }
...
//    sHtml = sHtml+ "<input type='submit' value='" + button_name + "'></form>";
    ...
    sHtml.form = sHtml.form + "<script>document.forms['alipaysubmit'].submit();</script>";
